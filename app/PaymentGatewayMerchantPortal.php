...
class PaymentGatewayMerchantPortal extends Model {
  ...
  function setcustomer($firstname,$lastname,$address1,
        $city,
        $state,
        $country,$zip,$customer_vault) {
    $this->customer['firstname'] = $firstname;
    $this->customer['lastname']  = $lastname;
    $this->customer['address1']  = $address1;
    $this->customer['city']      = $city;
    $this->customer['state']     = $state;
    $this->customer['country']   = $country;
    $this->customer['zip']   = $zip;
    $this->customer['customer_vault'] = $customer_vault;
  }

  function setBilling($firstname,$lastname,$company,$address1,$address2,
        $city,
        $state,
        $zip,
        $country,
  ...
  function saveCustomer($ccnumber, $ccexp) {
    ...
    $query .= "ccexp=" . urlencode($ccexp) . "&";
    // Billing Information
    $query .= "firstname=" . urlencode($this->customer['firstname']) . "&";
    $query .= "lastname=" . urlencode($this->customer['lastname']) . "&";
    $query .= "address1=" . urlencode($this->customer['address1']) . "&";
    $query .= "city=" . urlencode($this->customer['city']) . "&";
    $query .= "state=" . urlencode($this->customer['state']) . "&";
    $query .= "country=" . urlencode($this->customer['country']) . "&";
    $query .= "zip=" . urlencode($this->customer['zip']) . "&";
    $query .= "customer_vault=" . urlencode($this->customer['customer_vault']);

    return $this->_doPost($query);
  }
  function payCustomer($amount,$vault_id,$stored_credential_indicator) {
    ...
    $query .= "stored_credential_indicator=" . urlencode($stored_credential_indicator) . "&";
